...
public:
    ...
    void sqrtsd(Xmm d, Xmm s){ sseRR(0x51, d, s); }     // d = sqrt(s)
    void minsd(Xmm d, Xmm s) { sseRR(0x5D, d, s); }
    void maxsd(Xmm d, Xmm s) { sseRR(0x5F, d, s); }
    void xorpd(Xmm d, Xmm s) { u8(0x66); rex(false,d,s); u8(0x0F); u8(0x57); modrmReg(d,s); } // d ^= s (zero via d,d)
